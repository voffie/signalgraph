...
const Inventory = Consumer.make({
  name: "inventory-order-created",
  message: OrderCreated,
  handler: (payload) => Effect.sync(() => {
    console.log("Inventory got:", payload.orderId);
  })
});

const topology = Topology.make({ consumers: [Billing, Inventory] });

const program = Effect.gen(function* () {
  yield* Messaging.start(topology);
  yield* Messaging.publish(OrderCreated, { orderId: "123" });
});
