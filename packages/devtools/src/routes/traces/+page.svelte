<script lang="ts">
  ...
  const filteredIds: Set<string> | null = $derived(
    search.trim()
      ? new Set(
          data.traces.filter((n) => matchSearchTrace(n, search, searchField)).map((n) => n.traceId),
        )
      : null,
  );
...
{#if data.error}
  <div class="flex flex-1 items-center justify-center p-8 text-center">
    <div>
      <h2 class="text-text m-0 text-sm font-semibold">Unable to load traces</h2>
...
{:else if data.traces.length === 0}
  <div class="flex flex-1 items-center justify-center p-8 text-center">
    <p class="text-text2 text-xs">No traces found.</p>
  </div>
{:else}
        ...
        <div class="border-border_hi bg-panel rounded-lg border">
          <table class="w-full border-separate border-spacing-0 text-left text-xs">
            <thead>
              <tr>
                <th
                  class="bg-panel text-text2 rounded-tl-lg px-4 py-2.5 text-[10.5px] font-medium tracking-wide uppercase shadow-[0_1px_0_0_var(--color-border_hi)]"
                  >Trace ID</th>
                <th
                  class="bg-panel text-text2 px-4 py-2.5 text-[10.5px] font-medium tracking-wide uppercase shadow-[0_1px_0_0_var(--color-border_hi)]"
                  >Root service</th>
                <th
                  class="bg-panel text-text2 px-4 py-2.5 text-[10.5px] font-medium tracking-wide uppercase shadow-[0_1px_0_0_var(--color-border_hi)]"
                  >Operation</th>
                ...
                <th
                  ...
                  >Started</th>
                <th
                  class="bg-panel text-text2 rounded-tr-lg px-4 py-2.5 text-[10.5px] font-medium tracking-wide uppercase shadow-[0_1px_0_0_var(--color-border_hi)]"
                  >Status</th>
              </tr>
            </thead>
            <tbody>
              {#each data.traces as trace (trace.traceId)}
                <tr class="transition-colors hover:bg-white/4">
                  ...
                  <td
                    class="border-border/40 text-text max-w-50 truncate border-b px-4 py-3 font-medium"
                    >{trace.rootService}</td>
                  <td class="border-border/40 text-text2 max-w-50 truncate border-b px-4 py-3"
                    >{trace.rootOperation}</td>
                  <td
                    class="border-border/40 text-text border-b px-4 py-2 font-['JetBrains_Mono'] tabular-nums">
                  ...
                  <td class="border-border/40 border-b px-4 py-3">
                    <span
                      class={`inline-flex items-center rounded-full px-2 py-0.5 text-[10.5px] font-medium capitalize ${
                        trace.status === "error"
